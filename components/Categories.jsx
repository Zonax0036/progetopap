...
export default function CategoriaMenu() {
  ...
  // Utiliza um hook personalizado para obter as categorias e distrito de carregamento
  const { categorias, loading } = useCategorias();

  // Função para verificar se uma categoria está ativa com base na URL atual
  const isAtiva = id => router.query.categoria === String(id);
    ...
    <div className="bg-gray-100 shadow-sm border-b border-gray-200">
      <div className="container mx-auto px-4">
        ...
        <div className="flex overflow-x-auto py-3 scrollbar-hide space-x-2">
          ...
          {!loading &&
            categorias.map(cat => (
              <Link key={cat.id} href={`/?categoria=${cat.id}`} legacyBehavior>
                <a className={`category-btn ${isAtiva(cat.id) ? 'bg-blue-600 text-white' : ''}`}>
                  {cat.nome}
                </a>
              </Link>
            ))}
